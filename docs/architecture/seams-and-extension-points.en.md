...
!!! note "Durable execution is not a new seam"
    `RunRequest.durable=True` opts into auto-checkpoint + auto-resume for retryable errors. This is a runtime-level behavior decoration — **not a new seam**. See [api-reference.en.md §8 Durable execution](../reference/api-reference.en.md#durable-execution).
...
- [Developer Guide](../getting-started/developer-guide.en.md)
