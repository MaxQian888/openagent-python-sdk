...
!!! note "Durable execution 不是新 seam"
    `RunRequest.durable=True` 启用的自动 checkpoint + retryable 错误自动恢复是 runtime 层面的行为装饰，**不是新的 seam**。细节见 [api-reference.md §8 Durable execution](../reference/api-reference.md#durable-execution)。
