...
!!! note "executor_metadata in tool.succeeded"
    `executor_metadata` is an optional field carrying executor-side runtime metadata, for example: `RetryToolExecutor`'s `retry_attempts`, `SafeToolExecutor`'s `timeout_ms`, `CompositeExecutionPolicy`'s `decided_by`. Subscribers that only read `tool_id` / `result` are unaffected.

## Streaming Projection

The following events are also projected to `RunStreamChunk` (via `openagents/runtime/stream_projection.py`), and can be consumed through the streaming API — see [stream-api.md](stream-api.md):

| Event | RunStreamChunkKind |
|---|---|
...
| `tool.succeeded` | `TOOL_FINISHED` |
| `tool.failed` | `TOOL_FINISHED` |
| `validation.retry` | `VALIDATION_RETRY` |
| `artifact.emitted` | `ARTIFACT` |

Other events (such as `session.run.started`, `context.assemble.*`, `memory.*`, etc.) are not projected to the stream but can be subscribed to via the event bus.
