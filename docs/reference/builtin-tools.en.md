...
For an overview of the tool system architecture, see [Plugin Development Guide](../plugins/plugin-development.md). For configuring `tool_executor` (permission control, retry policies), see [Configuration Reference](../configuration/configuration.md).
...
- **`http_request`**: Does not automatically retry on 5xx errors. Configure the `retry` tool_executor for retry behaviour (see [Configuration Reference](../configuration/configuration.md)).
...
- [Configuration Reference](../configuration/configuration.md) — Full JSON schema for `tool` and `tool_executor`
- [Plugin Development Guide](../plugins/plugin-development.md) — Writing a custom tool plugin
- [Seams & Extension Points](../architecture/seams-and-extension-points.md) — Decision tree for the `tool` seam
