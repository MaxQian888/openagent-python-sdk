...
如需了解工具系统的整体架构，参见[插件开发指南](../plugins/plugin-development.md)。如需配置 `tool_executor`（权限控制、重试策略），参见[配置参考](../configuration/configuration.md)。
...
- **`http_request`**：不对 5xx 错误自动重试。如需重试，配置 `retry` tool_executor（见[配置参考](../configuration/configuration.md)）。
...
- [配置参考](../configuration/configuration.md) — tool 和 tool_executor 的完整 JSON schema
- [插件开发指南](../plugins/plugin-development.md) — 自定义工具插件开发
- [Seams 与扩展点](../architecture/seams-and-extension-points.md) — tool seam 的决策树
