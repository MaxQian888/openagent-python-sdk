...
Event Bus 在配置文件的顶层 `events` 字段配置，参见[配置参考](../configuration/configuration.md)。
...
- [配置参考](../configuration/configuration.md) — `logging` 块和 `events` 块的完整 JSON schema
- [插件开发指南](../plugins/plugin-development.md) — 自定义 Event Bus 插件
- [Seams 与扩展点](../architecture/seams-and-extension-points.md) — `events` seam 的决策树
