...
1. **Copy and analyze**:
   Copy the user-provided deck to `template.pptx`, then extract it with MarkItDown:
   ```bash
   markitdown template.pptx -o template.md
   ```
...
7. **Pack**: Repack the XML tree into a PPTX file. Validate, repair, condense XML, re-encode smart quotes.
   ...
   Always write to a local writable temp path first, then copy or move the result to the final path. Python's `zipfile` module uses `seek` internally, which fails on some mounted volumes. Writing to a local temp file avoids this.

## Output Structure

Copy the user-provided file to `template.pptx` in the working directory. This preserves the original and gives a predictable name for all downstream operations.
