...
```bash
markitdown output.pptx -o output.md
```

Check `output.md` for missing content, typos, and wrong order.

**Check for leftover placeholder text:**

```bash
rg -n -i "xxxx|lorem|ipsum|placeholder|this.*(page|slide).*layout" output.md
```

If `rg` returns results, fix them before declaring success.

### Verification Loop

1. Generate slides -> Extract text with `markitdown output.pptx -o output.md` -> Review content
...
```bash
markitdown slide-XX-preview.pptx -o slide-XX-preview.md
```

Check `slide-XX-preview.md` for missing content, placeholder text, and the page number badge.
