...
### Requirement: MockLLMClient enters coverage scope

`openagents/llm/providers/mock.py` SHALL be covered by unit tests that exercise every branch of `generate()`, `complete_stream()`, and the `_parse_prompt` helper. The module MUST NOT appear on the coverage omit list in `pyproject.toml` after this change. Total repository coverage MUST remain ≥ 90% after the change lands.

#### Scenario: mock.py is not in the coverage omit list
- **WHEN** `pyproject.toml` is inspected after this change lands
...
- **THEN** the total coverage is ≥ 90% and `mock.py` line coverage is 100%
