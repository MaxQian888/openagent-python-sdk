...
Transport-level retry/backoff and typed error normalization shared by every built-in `HTTPProviderClient`-derived LLM provider (`openagents/llm/providers/_http_base.py`). Centralizes classification of non-200 responses and connection-layer exceptions into the `LLMError` hierarchy (`openagents/errors/exceptions.py`), so downstream seams (retry policies, execution policy, UIs) can distinguish rate-limit vs transient vs malformed-response failures without parsing message strings. Also owns the `LLMOptions` config surface for retry / extra-headers.
