...
Lifecycle, connection-mode selection, pre-flight installation/connectivity checks, in-flight deduplication, and structured event emission for MCP-backed tools. Owned by `openagents/plugins/builtin/tool/mcp_tool.py` and tested via `tests/unit/test_mcp_tool.py`.

This capability constrains how the SDK's built-in MCP bridge opens sessions against Model Context Protocol servers (stdio subprocesses and HTTP/SSE endpoints), when it validates configuration, how it coalesces duplicate work, and what it tells operators through events. The overarching invariant is: the default mode MUST preserve the `anyio` cancel-scope safety guarantee that a dying MCP subprocess cannot cancel whatever the caller does next.
...
- **THEN** preflight raises `PermanentToolError` naming the tool id and the probe result, and `DefaultRuntime.run()` returns a `RunResult` with `stop_reason=StopReason.FAILED` without invoking the pattern
